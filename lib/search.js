...
export default async function searchRecipeByKeyword(keyword) {
  const client = await connectToDatabase();
  const db = client.db();

  const session = await getServerSession();

  // Split the keyword string into an array of individual keywords
    ...
    .aggregate([
      {
        $match: {
          $or: [
            { recipe_name: { $in: keywords.map((k) => new RegExp(k, "i")) } },
            {
        ...
        $and: [
          {
            $or: [
              ...
              { email: { $in: keywords.map((k) => new RegExp(k, "i")) } },
            ],
          },
